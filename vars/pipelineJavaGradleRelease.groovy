    ...
    pipeline {
        agent {
            docker {
                ...
                args "-u devops:devops --privileged -e PATH=${defaultPath} -v /app/maven/.m2:/home/devops/.m2 -v /app/maven/.gradle:/home/devops/.gradle -v /var/run/docker.sock:/var/run/docker.sock"
            }
        }
        options {
            timestamps()
        ...
        environment {
            ...
            ARTIFACTID = sh(script: "gradle peroperties > gradle.prop && cat gradle.prop|grep 'name'|awk '{print \$2}'", returnStdout: true).trim()
            VERSION = sh(script: "cat gradle.prop| grep 'version' | awk '{print \$2}'", returnStdout: true).trim()
            GROUPID = sh(script: "cat gradle.prop| grep 'group' | awk '{print \$2}'", returnStdout: true).trim()
            ARTIFACT_PACKAGING = sh(script: "if [[ \$(cat gradle.prop|grep 'jar') != '' ]]; then echo 'jar'; fi", returnStdout: true).trim()
        ...
        // parameters {
            // string(defualtValue: "", description: 'GIT branch or tag from upstream job', name: 'GIT_BRANCH_TAG')
        // }
        ...
        stages {
            ...
            // stage("Unit Test") {
                ...
                // steps {
                    // script {
                        // unitTestDriverObj = new UnitTestDriver(this, 'UnitTesterGradle')
                    ...
                    // stage ("Code Quality Scan") {
                        ...
                        // stages {
                            // stage("Code Scan Operations") {
                                // steps {
                                    // script {
                                        // codeScanDriverObj = new CodeScanDriver(this, 'CodeScannerGradle')
                                    // }
                                // }
                            // }
                        // }
                    ...
                    // stage ("Fortify Scan") {
                        ...
                        // stages {
                            // stage ("Fortify Scan Operations") {
                                // steps {
                                    // script {
                                        // fortifyScanDriverObj = new FortifyScanDriver(this, 'FortifyScannerGradle')
            ...
            // stage ("Upload to artifactory") {
                ...
                // steps {
                    // script {
                        // artifactoryUploadDriverObj = new ArtifactoryUploadDriver(this, 'ArtifactoryUploaderGradle')
